...
public class OIDCAuthenticationProvider implements AuthenticationProvider {
    ...
    if (authentication instanceof PendingOIDCAuthenticationToken) {
      ...
      if (userInfo == null) {
        ...
        if (!Strings.isNullOrEmpty(userInfo.getSub())
            && !userInfo.getSub().equals(token.getSub())) {
      ...
      return createAuthenticationToken(token, Set.of(new SimpleGrantedAuthority("ROLE_USER")),
          userInfo);
    }
  ...
  /**
   * Override this function to return a different kind of Authentication, processes the authorities
   * differently, or do post-processing based on the UserInfo object.
   *
   * @param token
   * @param authorities
   * @param userInfo
